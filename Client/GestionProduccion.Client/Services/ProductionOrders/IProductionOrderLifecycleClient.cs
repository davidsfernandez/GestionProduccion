...
public interface IProductionOrderLifecycleClient
{
    ...
    Task<ApiResponse<BulkUpdateResult>> BulkAdvanceStageAsync(List<int> orderIds, CancellationToken ct = default);
    Task<ApiResponse<bool>> ChangeStageAsync(int orderId, ProductionStage newStage, string note, CancellationToken ct = default);        
    Task<ApiResponse<BulkUpdateResult>> BulkChangeStageAsync(List<int> orderIds, ProductionStage newStage, string note, CancellationToken ct = default);
    Task<ApiResponse<ProductionOrderDto>> UpdateStatusAsync(int orderId, ProductionStatus newStatus, string note, CancellationToken ct = default);    Task<ApiResponse<BulkUpdateResult>> BulkUpdateStatusAsync(List<int> orderIds, ProductionStatus newStatus, string note, CancellationToken ct = default);
