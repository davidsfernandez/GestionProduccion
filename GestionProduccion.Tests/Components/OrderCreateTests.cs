...
public class OrderCreateTests : TestContext
{
    ...
    public void OrderCreate_ShouldPopulateSizesDropdown_Statically()
    {
        ...
        // Assert - The UI changed from a single select#size to a dynamic table
        cut.WaitForState(() => cut.FindAll("table").Count > 0);

        var table = cut.Find("table");
        table.Should().NotBeNull();
        
        var addButton = cut.Find("button.btn-outline-primary");
        addButton.TextContent.Should().Contain("Adicionar Tamanho");
